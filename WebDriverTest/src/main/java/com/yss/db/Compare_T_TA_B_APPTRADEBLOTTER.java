...
public class Compare_T_TA_B_APPTRADEBLOTTER {

	public MyResponse compare_T_TA_B_APPTRADEBLOTTER(){
		Common.logInfo("compare_t_ta_b_apptradeblotter");
		
		MyResponse myResponse = new MyResponse();
		SimpleDateFormat sdf  = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss");
		File dir = new File("E://T_TA_B_APPTRADEBLOTTER/");
		//获取文件夹下所有文件名称
		...
		File oldFile = new File("E://T_TA_B_APPTRADEBLOTTER/T_TA_B_APPTRADEBLOTTER-"+oldTime+".json");
		File newFile = new File("E://T_TA_B_APPTRADEBLOTTER/T_TA_B_APPTRADEBLOTTER-"+newTime+".json");
		try {
			...
			if(oldStringArray.length != newStringArray.length){
				...
				Common.logError("交易申请检查笔数不同!新版"+strErr+num+"笔");
				myResponse.failed("交易申请检查笔数不同!新版"+strErr+num+"笔");	
			}
			//获取最大的交易笔数
			...
			for(int i = 0; i < stringLength; i++){
				...
				if(newStringArray.length==i){
					Common.logError("新交易申请检查中没有第"+(i+1)+"笔交易!");
					return myResponse.failed("新交易申请检查中没有第"+(i+1)+"笔交易!");
				}
				//判断交易的总笔数
				if(oldStringArray.length==i){
					Common.logError("旧交易申请检查中没有第"+(i+1)+"笔交易!");
					return myResponse.failed("旧交易申请检查中没有第"+(i+1)+"笔交易!");
				}
				...
				if(newStringSplitArray.length != oldStringSplitArray.length ){
					if(!isNumEqual){
						Common.logError("交易检查时第"+(i+1)+"笔交易申请"+strErr+"或字段发生错误!(新)");
						return myResponse.failed("交易检查时第"+(i+1)+"笔交易申请"+strErr+"或字段发生错误!(新)");
					}
					Common.logError("第"+(i+1)+"笔交易申请检查发生错误,缺少字段!");
					myResponse.failed("第"+(i+1)+"笔交易申请检查发生错误,缺少字段!");
				}
				//获取最大的字段数目
				...
				for(int j = 0; j < stringSplitLength; j++){
					if( !newStringSplitArray[j].equals(oldStringSplitArray[j]) ){
						if(!isNumEqual){
							Common.logError("交易检查时没有第"+(i+1)+"笔交易申请检查!");
							return myResponse.failed("交易检查时没有第"+(i+1)+"笔交易申请检查!");
						}
						Common.logError("第"+(i+1)+"笔交易申请检查的字段"+newStringSplitArray[j]+"发生了错误(新)");
						myResponse.failed("第"+(i+1)+"笔交易申请检查的字段"+newStringSplitArray[j]+"发生了错误(新)");
						}
					}
				}
		} catch (Exception e) {
